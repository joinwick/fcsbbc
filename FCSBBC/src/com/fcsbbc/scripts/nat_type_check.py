...
#         print('tranid =', tranid, ':', 'tranid.upper() =', tranid.upper())
#         print('binascii.b2a_hex(buf[4:20]).upper() =', binascii.b2a_hex(buf[4:20]).upper())
        tranid_match = tranid.upper() == str(binascii.b2a_hex(buf[4:20]).upper(), encoding = "utf-8")
#         print('bind_resp_msg =', bind_resp_msg, ":", 'tranid_match =', tranid_match)
        if bind_resp_msg and tranid_match:
            ...
            base = 20
            while len_remain:
#                 print('binascii.b2a_hex(buf[base:(base + 2)]) = ', binascii.b2a_hex(buf[base:(base + 2)]))
                attr_type = str(binascii.b2a_hex(buf[base:(base + 2)]), encoding = "utf-8")
                attr_len = int(binascii.b2a_hex(buf[(base + 2):(base + 4)]), 16)
#                 print('attr_type =', attr_type, ':', 'attr_len =', attr_len)
...
def get_nat_type(s, source_ip, source_port, stun_host = None, stun_port = 3478):
    ...
    resp = False
    if stun_host:
#         print('stun_host is not None:', stun_host)
        ret = stun_test(s, stun_host, port, source_ip, source_port)
        resp = ret['Resp']
    else:
#         print('Not specify stun_host, stun_host chosen randomly!')
        for stun_host in stun_servers_list:
#             print('Using stun_host:', stun_host, ', source_ip:', source_ip)
...
#     print('ExternalIP =', exIP, ':', 'ChangedIP =', changedIP)
#     print('Test1(Direct): ret =', ret)
    if ret['ExternalIP'] == source_ip:
#         print('ExternalIP equals source_ip!')
        changeRequest = ''.join([ChangeRequest, '0004', "00000006"])
#         print('changeRequest =', changeRequest, ', stun_host =', stun_host, ', source_ip =', source_ip)
...
#         print('Test2: ret =', ret)
        ...
        if ret['Resp']:
            ...
            log.debug("Do Test1")
#             print('changedIP =', changedIP, ', source_ip =', source_ip)
            ret = stun_test(s, changedIP, changedPort, source_ip, source_port)
#             print('Test1(Call): ret =', ret)
            log.debug("Result: %s", ret)
            if not ret['Resp']:
                ...
                if exIP == ret['ExternalIP'] and exPort == ret['ExternalPort']:
                    ...
                    log.debug("Do Test3")
#                     print('changePortRequest =', changePortRequest, ', changedIP =', changedIP, ', source_ip =', source_ip)
...
#                     print('Test3: ret =', ret)
                    ...
                    typ = SymmetricNAT
#     print('nat type =', typ, ':', 'ret =', ret)
    return typ, ret


def get_ip_info(source_ip, source_port, stun_host, stun_port):
...
def get_nat_classfication(source_ip, source_port, stun_host, stun_port):
    nat_tuple = ()
    nat_tuple = get_ip_info(source_ip, source_port, stun_host, stun_port)
    if nat_tuple:
        return nat_tuple[0]
...
#     filename = sys.argv[1]
#     print(filename)
    nat_type = get_nat_classfication("0.0.0.0", 54320, None, 3478)
    # with open(filename, 'w') as f:
    #     f.write(result)
    print(nat_type)
# print(get_nat_classfication(DEFAULTS['source_ip'], DEFAULTS['source_port'], None, DEFAULTS['stun_port']))
# print(get_nat_classfication("0.0.0.0", 54320, None, 3478))